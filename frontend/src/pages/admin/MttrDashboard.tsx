    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-slate-400">{t('Loading remediation metrics...')}</p>
        </div>
      </div>
    );
  }
  ...
  if (!metrics) {
    return (
      <div className="p-6 text-center text-slate-400">
        ...
        <p>{t('Failed to load MTTR analytics. Please ensure database is seeded.')}</p>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="border-b border-obsidian-800 pb-4">
        <h1 className="text-3xl font-bold font-mono text-transparent bg-clip-text bg-gradient-to-r from-cyber-cyan via-cyber-blue to-purple-500">
          {t('Remediation Analytics & MTTR')}
        </h1>
        <p className="text-slate-400 mt-2">
          {t('Monitor SLA compliance rates and Mean Time To Remediation (MTTR) comparisons between cyber-security patches and operational guidelines.')}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="bg-obsidian-950/20 border-obsidian-850">
          ...
          <CardContent>
            ...
            <span className="text-[10px] text-slate-500 block mt-1">{t('Target: ')}{metrics.targets.security_target_hours} Hrs (3 Days)</span>
          </CardContent>
        </Card>
        ...
        <Card className="bg-obsidian-950/20 border-obsidian-850">
          ...
          <CardContent>
            ...
            <span className="text-[10px] text-slate-500 block mt-1">{t('Target: ')}{metrics.targets.operational_target_hours} Hrs (15 Days)</span>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-obsidian-950/10 border border-obsidian-850 rounded-xl p-5 flex flex-col justify-center">
          <span className="text-xs text-slate-500 font-bold uppercase tracking-wider block mb-1">{t('Total Compliance Load')}</span>
          <div className="text-4xl font-bold font-mono text-foreground">{metrics.total_active_maps}</div>
          <p className="text-xs text-slate-400 mt-2">{t('Active Multi-Agent Remediation Plans registered in the ledger.')}</p>
        </div>
        <div className="bg-obsidian-950/10 border border-obsidian-850 rounded-xl p-5 flex flex-col justify-center">
          <span className="text-xs text-slate-500 font-bold uppercase tracking-wider block mb-1">{t('Active Gaps / Remediation')}</span>
          <div className="text-4xl font-bold font-mono text-orange-500">{metrics.open_maps}</div>
          <p className="text-xs text-slate-400 mt-2">{t('Tasks currently in "open" or "in_progress" status.')}</p>
        </div>
        <div className="bg-obsidian-950/10 border border-obsidian-850 rounded-xl p-5 flex flex-col justify-center">
          <span className="text-xs text-slate-500 font-bold uppercase tracking-wider block mb-1">{t('SLA Breaches')}</span>
          <div className="text-4xl font-bold font-mono text-red-500">{metrics.overdue_maps}</div>
          <p className="text-xs text-slate-400 mt-2">{t('Active tasks currently exceeding their regulatory deadlines.')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 glass-dark rounded-xl p-6 border border-obsidian-850 bg-obsidian-950/20">
          <div className="flex items-center gap-2 mb-6">
            ...
            <div>
              <h3 className="font-bold text-sm">{t('Monthly MTTR Trend Comparison')}</h3>
              <p className="text-xs text-slate-500 mt-0.5">{t('Remediation time improvements (in Hours) over previous months')}</p>
            </div>
          </div>
        ...
        <div className="glass-dark rounded-xl p-6 border border-obsidian-850 bg-obsidian-950/20 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-bold text-sm">{t('Remediation Targets & SLAs')}</h3>
            </div>
            
            <p className="text-xs text-slate-400 leading-relaxed mb-4">
              {t("To comply with the bank's strict internal risk directives, security alerts must be patched and closed within 72 hours, while operational revisions must settle within 15 days.")}
            </p>
            
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-xs font-semibold mb-1">
                  <span className="text-slate-400">{t('Security SLA Met (72h)')}</span>
              ...
              <div>
                <div className="flex justify-between text-xs font-semibold mb-1">
                  <span className="text-slate-400">{t('Operational SLA Met (15d)')}</span>
